...
# 測試配置
UAV_COUNTS = list(range(4, 7))  # 4, 5, 6
...
plot_dir = Path(f"batch_test_4to6_plots_{timestamp}")
...
for uav_count in UAV_COUNTS:
    for seed in SEEDS:
        ...
        try:
            ...
            print(f"[OK] TSP 完成: Makespan={tsp_data['makespan']}, Distance={tsp_data['total_distance']}")
...
csv_filename = f"oblap_vs_tsp_4to6_results_{csv_timestamp}.csv"
...
if oblap_results and tsp_results:
    ...
    # 按 UAV 數量分組統計
    for uav_count in UAV_COUNTS:
        uav_oblap = [r for r in oblap_results if r['uav_count'] == uav_count]
        uav_tsp = [r for r in tsp_results if r['uav_count'] == uav_count]
        
        if uav_oblap and uav_tsp:
            oblap_makespans = [r['monitoring_complete'] for r in uav_oblap if r.get('monitoring_complete')]
            tsp_makespans = [r['makespan'] for r in uav_tsp if r.get('makespan')]
            
            if oblap_makespans and tsp_makespans:
                avg_oblap = sum(oblap_makespans) / len(oblap_makespans)
                avg_tsp = sum(tsp_makespans) / len(tsp_makespans)
                improvement_pct = ((avg_tsp - avg_oblap) / avg_tsp) * 100
                print(f"  UAV={uav_count}: OBLAP平均={avg_oblap:.1f}, TSP平均={avg_tsp:.1f}, 改善={improvement_pct:+.1f}%, 成功={len(uav_oblap)}/15")
