...
class ImprovedKMeansGATSPPlanner(Planner):
    ...
    def solve_hungarian_assignment(self, agents_pos: List[Tuple], tasks_pos: List[Tuple]) -> Optional[List[Tuple[int, int]]]:
        """為 greedy-dynamic 策略提供一個標準的匈牙利分配方法（最小化總和）。"""
    ...
    def plan_paths_for_points(self, points_to_cover: List[Tuple], num_drones: int, start_positions: List[Tuple], params: Optional[Dict] = None) -> List[List[Tuple]]:
        """
        【已修復】為一組給定的點進行分群和 TSP 路徑規劃。
        在重規劃時，使用以無人機當前位置為初始中心的 K-Means 來保證負載均衡。
        """
        ...
        if len(points_array) < num_drones:
            ...
            # 情況 A: 初始規劃，所有無人機從同一個點出發，使用標準 K-Means
            ...
            for i, label in enumerate(kmeans.labels_):
                clusters_map[label].append(tuple(points_array[i]))
            clusters = list(clusters_map.values())

        else:
            # 情況 B: 【核心修正】重規劃，無人機在不同位置。
            # 使用無人機的當前位置作為 K-Means 的初始中心點，以確保負載均衡。
            
            # 確保初始中心點的數量和 K 值匹配
            initial_centroids = np.array(start_positions)
            if len(initial_centroids) != num_drones:
                # 如果不匹配 (理論上不應發生，但作為防禦性程式設計)，則隨機選擇
                indices = np.random.choice(len(points_array), num_drones, replace=False)
                initial_centroids = points_array[indices]
